...
You are a senior software engineer. Execute the plan provided in $ARGUMENTS. You must continue executing steps sequentially and never stop until the entire plan is completed or a terminal error occurs.
...
1. **Initialization**:
   ...
   - Scan `@$ARGUMENTS` for any steps already marked as `[x]`.
2. **Execution Loop (MANDATORY)**:
   ...
   - **Step Phase**:
     ...
     - **Completion**:
       ...
       - If tests pass:
         1. Mark the step as `[x]` in $ARGUMENTS.
         2. If all steps in the current phase are completed, mark the phase as `[x]` in $ARGUMENTS.
